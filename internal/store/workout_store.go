...
type Workout struct {
	ID              int            `json:"id"`
	UserID          int            `json:"user_id"` // Added user_id to associate workouts with users
...
// PostgresWorkoutStore satisfies this interface
...
func (pg *PostgresWorkoutStore) CreateWorkout(workout *Workout) (*Workout, error) {
	...
	// If any errors occur while data is being manipulated, only then we rollback (As errors are collected and validated by tx)
	...
	// Entries also need to be inserted
...
func (pg *PostgresWorkoutStore) GetWorkoutByID(id int64) (*Workout, error) {
	...
	// Now fetching entries with this workout ID
	entryQuery := `
...
func (pg *PostgresWorkoutStore) UpdateWorkout(workout *Workout) error {
	...
	// WE have to also check if any rows were affected
	...
	// Deleting entries and adding again wtih new information from Workout struct (full patch update)
